...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        #Get Counts for all of the diff names
        filtered_df = filtered_df.groupby('Launch Site', as_index = False)['class'].value_counts()
        print(filtered_df)
        # return the outcomes piechart for a selected site
        
        #Check specified name and select only that site
        if entered_site == 'CCAFS LC-40':
            filtered_df = filtered_df[filtered_df['Launch Site'] == 'CCAFS LC-40']
            print(filtered_df)
            #Make fig w filtered data
